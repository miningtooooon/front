  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="relative w-full aspect-square max-w-[320px] flex items-center justify-center">
        ...
        {session.isActive && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            ...
            <div
              className="absolute w-[95%] h-[95%] border border-blue-500/10 rounded-full animate-reverse-spin-slow"
        ...
        <div
          className={`relative z-10 p-8 glass rounded-[40px] border-purple-500/40 glow-purple flex flex-col items-center justify-center transition-all duration-700 overflow-hidden ${
            session.isActive
              ? 'scale-105 shadow-[0_0_50px_rgba(168,85,247,0.5)] border-purple-400'
              : 'scale-100'
          }`}
        >
          ...
          <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1 opacity-20">
            ...
            <div
              className={`h-1 w-8 rounded-full ${session.isActive ? 'bg-blue-500 animate-pulse' : 'bg-gray-600'}`}
              style={{ animationDelay: '0.2s' }}
            ></div>
            <div
              className={`h-1 w-8 rounded-full ${session.isActive ? 'bg-purple-500 animate-pulse' : 'bg-gray-600'}`}
        ...
        />
        <Zap
          className={`absolute bottom-10 left-0 text-blue-400/40 transition-all duration-1000 ${
            session.isActive ? 'animate-pulse opacity-100 scale-125' : 'opacity-20 scale-100'
          }`}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 w-full">
        <div
          className={`glass p-4 rounded-2xl border-purple-500/20 transition-all duration-500 ${
            session.isActive ? 'bg-purple-500/5 border-purple-500/40' : ''
          }`}
        >
        ...
        <div
          className={`glass p-4 rounded-2xl border-purple-500/20 transition-all duration-500 ${
            session.isActive ? 'bg-blue-500/5 border-blue-500/40' : ''
          }`}
        >
          <p className="text-purple-400 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1">
            <Timer size={10} />
