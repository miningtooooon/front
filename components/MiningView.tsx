...
const DEFAULT_CONFIG: AppConfig = {
  ...
  miningRate: 10,
  // ✅ miningDuration is still SECONDS
  miningDuration: 3600,
  referralReward: 500,
  minWithdraw: 10000,
  exchangeRate: 1000,
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const now = Date.now();
      const elapsedSeconds = (now - user.miningSession.startTime!) / 1000;

      if (elapsedSeconds >= config.miningDuration) {
        // ✅ miningRate is GP/HR, duration is seconds => convert seconds to hours
